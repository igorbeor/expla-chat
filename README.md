...
- **Online vs Away.** Each user carries a `status` (`online` / `away`) exchanged over `status:change` → `status:changed`. The client tracks tab/window activity via the **Page Visibility API plus window `focus`/`blur`**: it emits `away` when the tab is hidden or the window loses focus, and `online` when it regains both (de-duplicated/debounced to avoid flicker). The chat header reflects this and the “All / Online only” contact filter keys on it — so presence is meaningful without a database.
